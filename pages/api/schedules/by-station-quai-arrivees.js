...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Query schedules filtered by arrival station and track (quai)
    const query = `
      SELECT * FROM schedules 
      WHERE (arrival_station = ? AND (track = ? OR track_assignments LIKE ?))
    ...
    console.error(`Error fetching arrival schedules for station ${station} and quai ${quai}:`, error);
