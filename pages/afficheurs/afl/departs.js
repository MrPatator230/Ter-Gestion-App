...
const StationsScroller = ({ stations, scrollDuration }) => {
  ...
  // Spacer element to create 87px gap between duplicated lists
  const spacerStyle = {
    display: 'inline-block',
    width: '87px',
  };

  // If no scrolling needed, show stations once without duplication
  ...
  return (
    <div lang="fr">
      ...
      <main className={styles.aflContainer} role="main" aria-label="Tableau des départs">
          ...
          {schedulesToDisplay.map((schedule, index) => {
            ...
            return (
              <li
                ...
                className={`${styles.scheduleRow} ${isEven ? styles.scheduleRowEven : styles.scheduleRowOdd} ${currentPage === 1 ? styles.secondPageLineHeight : ''}`}
                role="listitem"
              >
                ...
                <section className={styles.rightSection} aria-label="Voie">
                  ...
                  {statusCode === 'canceled' ? (
                    <svg viewBox="0 0 500 500" style={{ width: 'auto', height: 'auto' }} aria-hidden="true" focusable="false" role="img" xmlns="http://www.w3.org/2000/svg">
                    ...
                    if (!stationInfo || !stationInfo.locationType) {
                      return <div>{trackAssignments[schedule.id]?.[gare] || schedule.track || '-'}</div>;
                    }
                    const now = new Date();
                    const timeStr = getStationTime(schedule, gare, 'departure');
                    if (!timeStr) {
                    ...
                    if (stationInfo.locationType.toLowerCase() === 'ville') {
                      if (diffMinutes <= 20 && diffMinutes >= 0) {
                        return <div>{trackAssignments[schedule.id]?.[gare] || schedule.track || '-'}</div>;
                      }
                    } else if (stationInfo.locationType.toLowerCase() === 'interurbain') {
                      if (diffMinutes <= 720 && diffMinutes >= 0) {
                        return <div>{trackAssignments[schedule.id]?.[gare] || schedule.track || '-'}</div>;
                      }
                    }
                    return <div>-</div>;
        ...
        {currentPage === 1 && schedules.length > 0 && (
          <section className={styles.nextDaySection}>
            <p className={styles.nextDayText}>Les prochains départs auront lieu demain.</p>
          </section>
        )}
      </main>
    </div>
