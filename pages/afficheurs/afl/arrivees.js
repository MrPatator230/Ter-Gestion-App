...
export default function AFLArrivals() {
  ...
  useEffect(() => {
    ...
    async function fetchStationInfo() {
      try {
        ...
        if (isMounted) setStationInfo(null);
      }
    }
  ...
  useEffect(() => {
    ...
    async function fetchSchedules() {
      try {
        const res = await fetch('/api/schedules/by-station?station=' + encodeURIComponent(gare));
  ...
  // Page alternation every 15 seconds
  useEffect(() => {
  ...
  // Toggle status display every 2 seconds
  useEffect(() => {
    if (schedules.length < 5) {
      setShowStatus(true);
      return;
